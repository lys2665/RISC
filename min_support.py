...
def run_experiment_pipeline():
    ...
    for dataset in tqdm(DATASETS, desc="总体进度"):
        ...
        results = []
        for min_support in tqdm(PARAM_RANGE, desc=f"{dataset} 参数扫描", leave=False):
            ...
            try:
                ...
                print(f"数据集 {dataset} 参数 {min_support} 失败: {str(e)}")
                results.append({
                    'min_support': min_support,
                    'Purity': np.nan,
                    'NMI': np.nan,
