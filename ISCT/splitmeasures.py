...
def GiniIndex(label, label_after_split):
    ...
    discriminitive = gini_index - len(label_after_split) / len(label) * (
                1 - sum([(label_after_split.count(i) / len(label_after_split)) ** 2 for i in set(
            label_after_split)])) - (len(remain_label)) / len(label) * (
                                 1 - sum([(remain_label.count(i) / len(remain_label)) ** 2 for i in set(remain_label)]))
...
def gain_ratio(label, hit_index):
    ...
    entropy_after = -p_hit_majority * np.log2(p_hit_majority) - p_miss_majority * np.log2(
        p_miss_majority)  # Entropy after split
