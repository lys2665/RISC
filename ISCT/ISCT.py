...
class TreeNode:
    def __init__(self, pattern=None, gini=None, hit_index=None, left=None, right=None, sequential_data=None,
                 cluster_num=None):
        self.pattern = pattern
        self.gini = gini
    ...
    def kmeans_cluster(self, k, maxseqlength):
        # using RandomProjection.py
        seqs = self.get_x()

        rp = RandomProjection(seqs, max_feature_num=2048, max_feature_len=maxseqlength)
        features = rp.generate_features()
        feature_X = rp.generate_feature_vector(features)
...
if __name__ == '__main__':
    ...
    for data in dataset:
        ...
        for i in range(n):
            ...
            purity_list.append(tree_acc[0])
            nmi_list.append(tree_acc[1])
            f1_list.append(tree_acc[2])
            cluster_list.append(len(set(y_pred)))

        tree_dict['purity'] = np.mean(purity_list)
        # tree_dict['purity_var'] = np.var(purity_list)
        tree_dict['nmi'] = np.mean(nmi_list)
        # tree_dict['nmi_var'] = np.var(nmi_list)
        tree_dict['f1'] = np.mean(f1_list)
        # tree_dict['f1_var'] = np.var(f1_list)
        tree_dict['cluster'] = np.mean(cluster_list)
        # tree_dict['cluster_var'] = np.var(cluster_list)
        tree_dict['time'] = time2 - time1
        tree_results.append(tree_dict)
        print(tree_results)

    # Convert to DataFrame
    df = pd.DataFrame(tree_results)

    # Save to Excel
    df.to_excel('pca_result.xlsx', index=False)
    print("Results saved to tree_results.xlsx")